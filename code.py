...
class App:
    ...
    def __init__(self, seg_names: Sequence[str] = (), **kwargs: Any) -> None:
        # control
        self.curseg: int = 0
        self.seg_names: List[str] = list(seg_names)
        self.seg_order: List[List[Any]] = segment_order(self.seg_names)
        self.seg_count: int = len(self.seg_names)

        self.best_segs: List[List[float]] = kwargs.get("best_segs") or []
        self.split_times: List[float] = kwargs.get("split_times") or []
        ...
        self.game_name: str = kwargs.get("game_name") or ""
        self.run_category: str = kwargs.get("run_category") or ""
    ...
    def setup_labels(self) -> None:
        ...
        self.game_name_label = tk.Label(self.main_timer_tab,text=self.game_name, font=("Consolas", 12))
        self.run_category_label = tk.Label(self.main_timer_tab,text=self.run_category, font=("Consolas", 10))
    ...
    def _pause_main_timer(self) -> None:
        if self.main_timer_running():
            ...
            if len(self.latest_splits) == self.seg_count:
                print(
                    f"[{datetime.now().strftime('%H:%M:%S')}] 🏁 Finished the run at {formatA(self.main_timer)}."
                )
            else:
                print(
                    f"[{datetime.now().strftime('%H:%M:%S')}] Paused at {formatA(self.main_timer)}."
                )
    ...
    def _split(self) -> bool:
        ...
        if len(self.latest_splits) == self.seg_count:
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Splitted into the "
                f"{ordinal_num(len(self.latest_splits) + 1)} overall segment at {formatA(self.main_timer)}."
            )
        self.seg_info_cache = get_seg_list_info(self.seg_order, curseg=len(self.latest_splits))
        return True
    ...
    def _desplit(self) -> bool:
        ...
        if self.main_timer_running():
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Desplited back into the "
                f"{ordinal_num(len(self.latest_splits))} segment."
            )
