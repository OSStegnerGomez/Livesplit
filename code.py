...
def get_seg_list_info(seg_list,**kwargs):
    curseg = kwargs.get("curseg")
    info = {"list":seg_list}
    if curseg is None:
        ...
        for i in range(len(_list)):
           ...
           # print("[REST]",seg,left,right)
            _repr.append({
                ...
                "depth_ind": seg[0],
                "latest": latest_seg,
                "start": el_by_ind(latest_splits,seg_start_ind-1,"restr_neg_ind",default=0)
                })
        return _repr
        
best_segments = [
        [1583.29],[194,363.3,328.11,204,187.77,None#265.26
                   ],
        [37.15,18.6,43,26,63.14,86.89,18.9,100.16,28.25,117.56,76.73,38.34,63.31,15,131.56,60.33,
...
class App:
    def __init__(self,name="NEW",seg_names=[],**kwargs):
        ...
        self.latest_splits = []
        self.seg_info_cache = get_seg_list_info(self.seg_order,curseg=self.curseg+1)
    ...
    def setup_labels(self):
        ...
        self.best_seg_labels =[tk.Label(text="Cartman",font=("Consolas",8)) for i in range(3)]
        self.pb_seg_labels =[tk.Label(text="Cartman",font=("Consolas",8)) for i in range(3)]
        self.main_timer_label.place(relx=0.5, rely=0.82,anchor="center")
        self.seg_timer_label.place(relx=0.5, rely=0.9,anchor="center")
        [label.place(relx=0, rely=0.72+0.08*i) for i, label in enumerate(self.best_seg_labels)]
        [label.place(relx=0, rely=0.76+0.08*i) for i, label in enumerate(self.pb_seg_labels)]
    ...
    def update_labels(self):
        ...
        curseg_cursors = [next(
                (e for e in seg_repr if e["range"][1]>=self.curseg and e["depth_ind"][0]==i),None
                 ) for i in range(len(self.best_seg_labels))]

        for i,(cursor,best_l,pb_l) in enumerate(zip(
            ...
            try:
                ...
                print("Something went wrong, whilst extracting BEST and TARGET Times:")
                print(f"At Index {i} ->",e)
        ...
        for i,(left,right,bgframe) in enumerate(
            ...
            if self.curseg in range(seg_i[1][0],seg_i[1][1]+1) and (self.start is not None) and len(self.latest_splits)!=len(self.split_times):
            ...
            mid_text = (
                "" if not seg_r in curseg_cursors else (
                    (
                        f"{(self.main_timer-seg_r['text'][2]):+.1f}" if (
                                self.main_timer>seg_r['text'][2] or self.main_timer-seg_r['start'] > seg_r["best"]
                            ) else ""
                        ) if (
                        seg_r["depth_ind"][0]+1 == len(curseg_cursors)
                        ) else (
                        formatA(self.main_timer-seg_r['start'])
                        ))) if mid_timer is None else f"{mid_timer:+.1f}"
            right_text = (
                formatB(seg_r["time"]) if (self.main_timer-seg_r["start"])<seg_r["time"] else f"{(self.main_timer-seg_r['start']-seg_r['time']):+.1f}"
                ) if (
                len(self.latest_splits)!=len(self.split_times) and seg_r in curseg_cursors and seg_r["depth_ind"][0]+1 != len(curseg_cursors)
                ) else formatB(right_time)
            ...
            if len(self.latest_splits)==len(self.split_times):
                try:
                    super_seg_timer = self.main_timer-seg_r['start']-seg_r['time']
                    mid_text = (
                        formatA(self.main_timer-seg_r['start'])
                    ) if seg_r in curseg_cursors and seg_r != curseg_cursors[-1] else f"{mid_timer:+.1f}"
                    right_text = (
                        f"{super_seg_timer:+.1f}"
                    ) if seg_r in curseg_cursors and seg_r != curseg_cursors[-1] else formatB(right_time)
                    if seg_r in curseg_cursors and seg_r != curseg_cursors[-1]:
                        right_fg = (
                            "green" if self.main_timer-seg_r['start']>seg_r['best'] else "gold"
                            ) if super_seg_timer < 0 else "red"
                except TypeError as e:
                        print("Something went wrong, whilst extracting BEST and TARGET Times:")
                        print(f"-> TypeError:",e)
            
            #if random() < 0.001:
            #    print(f"[test] {seg_r['depth_ind'][0]=}")
            right.configure(text=f"{mid_text}   {right_text}",fg=right_fg)
